...
TEST(large_float, multiplication_zero) {
    LargeNumbers::LargeFloatImplementation<uint32_t> number("0.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("1.0"));
    EXPECT_EQ(number.toString(), "0.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("-1.0"));
    EXPECT_EQ(number.toString(), "0.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.1"));
    EXPECT_EQ(number.toString(), "0.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("-0.1"));
    EXPECT_EQ(number.toString(), "0.0");

    number = LargeNumbers::LargeFloatImplementation<uint32_t>("1.0");
    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.0"));
    EXPECT_EQ(number.toString(), "0.0");
...
TEST(large_float, multiplication_without_fraction) {
    LargeNumbers::LargeFloatImplementation<uint32_t> number("1.0");
    ...
    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("2.0"));
    EXPECT_EQ(number.toString(), "2.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("-2.0"));
    EXPECT_EQ(number.toString(), "-4.0");

    number = LargeNumbers::LargeFloatImplementation<uint32_t>("2.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("256.0"));
...
TEST(large_float, multiplication_with_fraction) {
    LargeNumbers::LargeFloatImplementation<uint32_t> number("0.1");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.1"));
    EXPECT_EQ(number.toString(), "0.01");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.01"));
    EXPECT_EQ(number.toString(), "0.0001");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("10000.0"));
    EXPECT_EQ(number.toString(), "1.0");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.0625"));
    ...
    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("10.0"));
    EXPECT_EQ(number.toString(), "0.00390625");
...
TEST(large_float, multiplication_mixed) {
    LargeNumbers::LargeFloatImplementation<uint32_t> number("1.1");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("1.1"));
    EXPECT_EQ(number.toString(), "1.21");

    number.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("-2.223"));
    EXPECT_EQ(number.toString(), "-2.68983");
}

LargeNumbers::LargeFloatImplementation<uint32_t> createNumber(const std::string &number) {
    LargeNumbers::LargeFloatImplementation<uint32_t> n(number);
    EXPECT_EQ(n.toString(), number);
    return n;
}

TEST(large_float, high_precision_multiplication_1) {
    LargeNumbers::LargeFloatImplementation<uint32_t>::setDecimalPrecision(100);
...
TEST(large_float, multiplication_large_numbers) {
    ...
    n1.multiply(LargeNumbers::LargeFloatImplementation<uint32_t>("0.0"));
